...
function BlogInfo(){
    ...
    const getAllBlogs = async () => {
        setloading(true);
        try {
          const productTemp = await getDoc(doc(fireDB, "blogPosts", params.id))
    ...
    const addComment = async () => {
      const userRef = collection(fireDB, "blogPosts/" + `${params.id}/` + "comment")
      try {
        await addDoc(
          userRef, {
          fullName,
    ...
    const getcomment = async () => {
      try {
        const q = query(
          collection(fireDB, "blogPosts/" + `${params.id}/` + "comment/"),
          orderBy('time')
        );
